...
IMAGE_PROMPT = input("Enter the kind of design you want: ")
...
image_user = Protocol("Request")

# Define a function that sends a message to the AI model agent every 360 seconds


@image_user.on_interval(360, messages=Request)
async def auto_complete(ctx: Context):
    ctx.logger.info(f"Asking AI model agent to generate an image of : {IMAGE_PROMPT}")
    await ctx.send(AI_MODEL_AGENT_ADDRESS, Request(text=IMAGE_PROMPT))

# Define a function to handle and log data received from the AI model agent


@image_user.on_message(model=Data)
