...
SENTIMENT_URL = "https://api-inference.huggingface.co/models/lxyuan/distilbert-base-multilingual-cased-sentiments-student"
...
async def get_completion(ctx: Context, sender: str, completion_text: str):
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            return
        message = response.json()[0][0]['label']
...
sentiment_agent = Protocol("Request")


@sentiment_agent.on_message(model=Request, replies={Data, Error})
async def handle_request(ctx: Context, sender: str, request: Request):
    # Log the request details
    ctx.logger.info(
        f"Got request from  {sender} to analyse: {request.text}")
