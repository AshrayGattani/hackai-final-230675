...
COMPLETE_THIS = input("Enter your Query: ")
...
gemini_user = Protocol("Request")

# Define a function that sends a message to the AI model agent every 360 seconds


@gemini_user.on_interval(360, messages=Request)
async def auto_complete(ctx: Context):
    ctx.logger.info(f"Asking AI model agent to answer: {COMPLETE_THIS}")
...
@gemini_user.on_message(model=Data)
