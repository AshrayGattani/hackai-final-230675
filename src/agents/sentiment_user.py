...
COMPLETE_THIS = input("Enter User Feeback: ")
...
sentiment_user = Protocol("Request")

# Define a function that sends a message to the AI model agent every 360 seconds


@sentiment_user.on_interval(360, messages=Request)
async def auto_complete(ctx: Context):
    ctx.logger.info(f"Asking AI model agent to analyze: {COMPLETE_THIS}")
...
@sentiment_user.on_message(model=Data)
