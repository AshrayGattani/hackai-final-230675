...
from messages.basic import Data, Request, Error, Con  # Import Basic messages
...
AI_MODEL_AGENT_ADDRESS = "agent1q0tnpzudk3cqtkj6jh4f5jqynps4xqdg05347fyal30y4wv33gpvv5xeky2"
...
context_user = Protocol("Request")

# Define a function that sends a message to the AI model agent every 360 seconds


@context_user.on_interval(360, messages=Con)
async def auto_complete(ctx: Context):
    ctx.logger.info(f"Asking AI model agent to answer: {QUESTION}")
    await ctx.send(AI_MODEL_AGENT_ADDRESS, Con(text=QUESTION,con = CONTEXT))

# Define a function to handle and log data received from the AI model agent


@context_user.on_message(model=Data)
